...
/// 命令行维护命令。
///
/// 站点没有「找回密码」的入口 —— 只有两个人用，加邮件找回不值当，
/// 而且多一个对外的入口就多一处可以被打的地方。
...
public record MaintenanceCommand(MaintenanceAction Action, string UserName = "", string? NewPassword = null) {
    ...
    /// <summary>这几个开关是给我们自己用的，不能混进 Host 的配置里。</summary>
    private static readonly string[] OwnFlags = [ResetFlag, PasswordFlag, ListFlag, "--help"];

    /// <summary>从命令行里解析出一条维护命令；没有就返回 null，照常启动站点。</summary>
    ...
    /// <summary>取 <c>--开关 值</c> 里的那个值；开关不在或后面没跟值时返回 null。</summary>
